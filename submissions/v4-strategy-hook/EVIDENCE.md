...
| Repository tests | `FOUNDRY_PROFILE=ci forge test --offline --force -vv` | 27 reported test/property entries passed |
| Fuzz | Fee conservation, 10,000 runs per suite in CI profile | Passed locally |
...
| Mainnet fork | `MAINNET_RPC_URL=… forge test --match-path test/V4StrategyHookFork.t.sol --force -vv` | 13/13 passed against official PoolManager; pinned block 25,664,100 plus a separate current-head smoke check |
